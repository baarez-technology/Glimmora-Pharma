...
const DOC_TYPE_COLORS: Record<DocType, string> = { SOP: "#a57865", Record: "#4a5e3a", "Audit Trail": "#4a8fa8", Validation: "#c9a84c", Report: "#a78bfa", Protocol: "#c0392b", Certificate: "#4a5e3a", Policy: "#8e7065", Other: "#b89888" };
...
export function EvidencePage() {
  ...
  function exportPack(pack: EvidencePack) {
    ...
    const html = `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><title>${pack.name}</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,Arial,sans-serif;font-size:13px;color:#0a1628;padding:40px;background:#fff}.header{border-bottom:2px solid #a57865;padding-bottom:20px;margin-bottom:24px}.logo-row{display:flex;align-items:center;justify-content:space-between;margin-bottom:16px}.logo{font-size:14px;font-weight:700;color:#a57865}h1{font-size:22px;font-weight:700;margin-bottom:6px}.purpose{font-size:13px;color:#475569;margin-bottom:16px}.meta-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:24px}.meta-card{padding:12px;border-radius:8px;background:#f8fafc;border:1px solid #e2e8f0}.meta-label{font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.5px;color:#b89888;margin-bottom:4px}.meta-value{font-size:14px;font-weight:700;color:#0a1628}.green{color:#4a5e3a}.red{color:#c0392b}table{width:100%;border-collapse:collapse;margin-bottom:24px}thead tr{background:#503e37}th{padding:10px 12px;text-align:left;font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.5px;color:#b89888;border:1px solid #6b5349}.part11{padding:12px 16px;border-radius:8px;background:rgba(99,102,241,.06);border:1px solid rgba(99,102,241,.2);font-size:11px;color:#4a8fa8;margin-bottom:20px}.footer{border-top:1px solid #e2e8f0;padding-top:16px;display:flex;justify-content:space-between}.footer-left,.footer-right{font-size:10px;color:#b89888;line-height:1.6}.footer-right{text-align:right}@media print{body{padding:20px}}</style></head><body><div class="header"><div class="logo-row"><span class="logo">${org}</span><span style="font-size:10px;font-weight:600;padding:3px 8px;border-radius:4px;background:rgba(14,165,233,.1);color:#a57865">GxP Evidence Pack</span></div><h1>${pack.name}</h1><p class="purpose">${pack.purpose}</p></div><div class="meta-grid"><div class="meta-card"><div class="meta-label">Total documents</div><div class="meta-value">${docs.length}</div></div><div class="meta-card"><div class="meta-label">Current / ready</div><div class="meta-value green">${cc}</div></div><div class="meta-card"><div class="meta-label">Missing / gaps</div><div class="meta-value ${mc > 0 ? "red" : "green"}">${mc}</div></div><div class="meta-card"><div class="meta-label">Areas covered</div><div class="meta-value" style="font-size:11px;font-weight:600">${areas}</div></div></div><div class="part11">\uD83D\uDD12 This evidence pack was prepared under 21 CFR Part 11 audit controls. Export event recorded in the immutable audit trail. Prepared by: <strong>${prepBy}</strong> \u00b7 Exported: <strong>${expDate} UTC</strong></div><table><thead><tr><th>Reference</th><th>Document title</th><th>Type</th><th>Area</th><th>Status</th><th>Version</th><th>Effective date</th><th>Compliance tags</th></tr></thead><tbody>${rows || '<tr><td colspan="8" style="text-align:center;padding:24px;color:#b89888;font-style:italic">No documents in this pack</td></tr>'}</tbody></table><div class="footer"><div class="footer-left">Document types: ${types}<br/>Pack ID: ${pack.id}<br/>Generated by ${org}</div><div class="footer-right">Exported: ${expDate} UTC<br/>Prepared by: ${prepBy}<br/>For regulatory inspection use only</div></div></body></html>`;
  ...
  return (
    <main id="main-content" aria-label="Evidence and document workspace" className="w-full max-w-[1440px] mx-auto space-y-6">
      ...
      <div role="tabpanel" id="panel-library" aria-labelledby="tab-library" tabIndex={0} hidden={activeTab !== "library"}>
        ...
        {/* Facets */}
        <section aria-label="Document filters" className={clsx("flex items-center gap-3 flex-wrap mb-4 p-4 rounded-xl border", isDark ? "bg-[#503e37] border-[#6b5349]" : "bg-[#f8fafc] border-[#e2e8f0]")}>
          ...
          {/* View toggle */}
          <div className={clsx("ml-auto flex items-center gap-1 p-1 rounded-lg", isDark ? "bg-[#3a2d28]" : "bg-[#f1f5f9]")}>
            <button type="button" aria-pressed={viewMode === "grid"} aria-label="Grid view" onClick={() => setViewMode("grid")} className={clsx("p-1.5 rounded-md transition-colors border-none cursor-pointer", viewMode === "grid" ? "bg-[#a57865] text-white" : "bg-transparent text-(--text-muted)")}>
              <LayoutGrid className="w-3.5 h-3.5" />
            </button>
            <button type="button" aria-pressed={viewMode === "list"} aria-label="List view" onClick={() => setViewMode("list")} className={clsx("p-1.5 rounded-md transition-colors border-none cursor-pointer", viewMode === "list" ? "bg-[#a57865] text-white" : "bg-transparent text-(--text-muted)")}>
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          {selectedDocs.size > 0 && (
            <div className="flex items-center gap-1">
              <span className="text-[12px] text-[#a57865]">{selectedDocs.size} selected</span>
        ...
        {allDocs.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3" role="list" aria-label="Documents grid">
            {filteredDocs.map((doc) => {
              ...
              return (
                <div key={doc.id} role="listitem" className={clsx("card cursor-pointer transition-all duration-150 hover:border-[#a57865]", isSel && "ring-2 ring-[#a57865] ring-offset-1")}>
                  <div className="card-body">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <input type="checkbox" className="w-4 h-4 accent-[#a57865] cursor-pointer" checked={isSel} onChange={() => toggleDocSelection(doc.id)} aria-label={`Select ${doc.title}`} />
                    </div>
                    ...
                    {doc.complianceTags.length > 0 && <div className="flex gap-1 flex-wrap">{doc.complianceTags.map((tag) => <span key={tag} className="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-[rgba(74,143,168,0.1)] text-[#4a8fa8]">{tag}</span>)}</div>}
                    <div className="flex items-center justify-between mt-3 pt-2 border-t" style={{ borderColor: isDark ? "#0f2039" : "#f1f5f9" }}>
                      ...
                      <div className="flex items-center gap-1.5">
                        {doc.findingId && <button onClick={() => navigate("/gap-assessment", { state: { openFindingId: doc.findingId } })} title={`Finding: ${doc.findingId}`} className="opacity-50 hover:opacity-100 border-none bg-transparent cursor-pointer"><Search className="w-3.5 h-3.5 text-[#a57865]" /></button>}
                        {doc.capaId && <button onClick={() => navigate("/capa", { state: { openCapaId: doc.capaId } })} title={`CAPA: ${doc.capaId}`} className="opacity-50 hover:opacity-100 border-none bg-transparent cursor-pointer"><ClipboardCheck className="w-3.5 h-3.5 text-[#4a5e3a]" /></button>}
                        {doc.eventId && <button onClick={() => navigate("/fda-483")} title="FDA 483 response" className="opacity-50 hover:opacity-100 border-none bg-transparent cursor-pointer"><FileWarning className="w-3.5 h-3.5 text-[#c0392b]" /></button>}
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="card overflow-hidden"><div className="overflow-x-auto">
            <table className="data-table" aria-label="Evidence document library">
              ...
              <thead><tr>
                <th scope="col"><input type="checkbox" className="w-4 h-4 accent-[#a57865]" checked={selectedDocs.size === filteredDocs.length && filteredDocs.length > 0} onChange={() => setSelectedDocs(selectedDocs.size === filteredDocs.length ? new Set() : new Set(filteredDocs.map((d) => d.id)))} aria-label="Select all" /></th>
                ...
                {filteredDocs.map((doc) => {
                  ...
                  return (
                    <tr key={doc.id} className={clsx(selectedDocs.has(doc.id) && (isDark ? "bg-[#071e38]" : "bg-[#eff6ff]"))}>
                      <td><input type="checkbox" className="w-4 h-4 accent-[#a57865]" checked={selectedDocs.has(doc.id)} onChange={() => toggleDocSelection(doc.id)} aria-label={`Select ${doc.title}`} /></td>
                      ...
                      <td className="text-[12px]" style={{ color: "var(--text-secondary)" }}>{doc.effectiveDate ? dayjs.utc(doc.effectiveDate).tz(timezone).format(dateFormat) : "\u2014"}{doc.expiryDate && dayjs.utc(doc.expiryDate).isBefore(dayjs()) && <div className="text-[10px] text-[#c0392b]">Expired</div>}</td>
                      <td><div className="flex gap-1 flex-wrap">{doc.complianceTags.map((tag) => <span key={tag} className="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-[rgba(74,143,168,0.1)] text-[#4a8fa8]">{tag}</span>)}</div></td>
                      <td><div className="flex items-center gap-1.5">
                        {doc.findingId && <button onClick={() => navigate("/gap-assessment", { state: { openFindingId: doc.findingId } })} title={`Finding: ${doc.findingId}`} className="opacity-50 hover:opacity-100 border-none bg-transparent cursor-pointer"><Search className="w-3.5 h-3.5 text-[#a57865]" /></button>}
                        {doc.capaId && <button onClick={() => navigate("/capa", { state: { openCapaId: doc.capaId } })} title={`CAPA: ${doc.capaId}`} className="opacity-50 hover:opacity-100 border-none bg-transparent cursor-pointer"><ClipboardCheck className="w-3.5 h-3.5 text-[#4a5e3a]" /></button>}
                        {doc.eventId && <button onClick={() => navigate("/fda-483")} title="FDA 483" className="opacity-50 hover:opacity-100 border-none bg-transparent cursor-pointer"><FileWarning className="w-3.5 h-3.5 text-[#c0392b]" /></button>}
                      </div></td>
                    </tr>
                  );
                })}
      ...
      <div role="tabpanel" id="panel-builder" aria-labelledby="tab-builder" tabIndex={0} hidden={activeTab !== "builder"}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
          {[{ step: 1, Icon: CheckSquare, color: "#a57865", title: "Select documents", desc: "Go to Document Library and check the documents you want in the pack." },
            { step: 2, Icon: Package, color: "#4a8fa8", title: "Name the pack", desc: 'Give the pack a name and purpose, e.g. "FDA 483 Response Pack \u2014 Mar 2026".' },
            { step: 3, Icon: Download, color: "#4a5e3a", title: "Export", desc: "Preview the pack metadata and export as a structured document list." }].map((s) => (
            <div key={s.step} className={clsx("rounded-xl p-4 border", isDark ? "bg-[#503e37] border-[#6b5349]" : "bg-[#f8fafc] border-[#e2e8f0]")}>
        ...
        {selectedDocs.size === 0 ? (
          <div className="card p-8 text-center"><Package className="w-10 h-10 mx-auto mb-2" style={{ color: "#6b5349" }} aria-hidden="true" /><p className="text-[12px]" style={{ color: "var(--text-secondary)" }}>No documents selected yet. Go to Document Library and select documents to include.</p><Button variant="ghost" size="sm" className="mt-2" onClick={() => setActiveTab("library")}>Go to Document Library</Button></div>
        ) : (
          <div className="card">
            <div className="card-header"><div className="flex items-center gap-2"><Package className="w-4 h-4 text-[#4a8fa8]" aria-hidden="true" /><span className="card-title">Selected for pack</span></div><Badge variant="blue">{selectedDocs.size} documents</Badge></div>
            <div className="card-body space-y-2">
              {Array.from(selectedDocs).map((docId) => { const doc = allDocs.find((d) => d.id === docId); if (!doc) return null; const DI = DOC_TYPE_ICONS[doc.type]; const ic = DOC_TYPE_COLORS[doc.type]; return (
                <div key={docId} className="flex items-center justify-between p-2.5 rounded-lg" style={{ background: isDark ? "#3a2d28" : "#f8fafc" }}>
      ...
      <div role="tabpanel" id="panel-dil" aria-labelledby="tab-dil" tabIndex={0} hidden={activeTab !== "dil"}>
        ...
        <div className={clsx("flex items-start gap-2 p-4 rounded-xl mb-6 border", isDark ? "bg-[rgba(245,158,11,0.06)] border-[rgba(245,158,11,0.15)]" : "bg-[#fffbeb] border-[#fde68a]")}>
          ...
          <div><p className="text-[12px] font-medium text-[#c9a84c]">DIL &mdash; Document Information List</p><p className="text-[11px] mt-0.5" style={{ color: "var(--text-secondary)" }}>Inspectors submit a DIL &mdash; a list of documents they want to review. This board tracks which documents have been retrieved and are ready.</p></div>
        </div>

        {/* Tiles */}
        <section aria-label="DIL statistics" className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
          <div className="stat-card" role="region" aria-label="Total requested"><div className="flex items-center gap-2 mb-2"><ClipboardList className="w-5 h-5 text-[#4a8fa8]" aria-hidden="true" /><span className="stat-label mb-0">Total requested</span></div><div className="stat-value">{allDocs.length}</div><div className="stat-sub">Documents in scope</div></div>
          <div className="stat-card" role="region" aria-label="Ready"><div className="flex items-center gap-2 mb-2"><CheckCircle2 className="w-5 h-5" style={{ color: currentCount > 0 ? "#4a5e3a" : "#8e7065" }} aria-hidden="true" /><span className="stat-label mb-0">Ready</span></div><div className="stat-value" style={{ color: currentCount > 0 ? "#4a5e3a" : "#8e7065" }}>{currentCount}</div><div className="stat-sub">Retrieved and ready</div></div>
          <div className="stat-card" role="region" aria-label="Missing"><div className="flex items-center gap-2 mb-2"><AlertCircle className="w-5 h-5" style={{ color: missingCount > 0 ? "#c0392b" : "#4a5e3a" }} aria-hidden="true" /><span className="stat-label mb-0">Missing</span></div><div className="stat-value" style={{ color: missingCount > 0 ? "#c0392b" : "#4a5e3a" }}>{missingCount}</div><div className="stat-sub">Not found or missing</div></div>
        </section>

        {/* Fulfillment by area */}
        <div className="card mb-4"><div className="card-header"><div className="flex items-center gap-2"><ClipboardCheck className="w-4 h-4 text-[#a57865]" aria-hidden="true" /><span className="card-title">DIL fulfillment by area</span></div></div><div className="card-body space-y-3">
          {DOC_AREAS.map((area) => {
            ...
            return (
              <div key={area}>
                <div className="flex items-center justify-between mb-1"><span className="text-[12px]" style={{ color: "var(--text-primary)" }}>{area}</span><div className="flex items-center gap-2"><span className="text-[11px]" style={{ color: "var(--text-muted)" }}>{curr}/{total}</span><span className="text-[12px] font-semibold" style={{ color: pct === 100 ? "#4a5e3a" : pct >= 60 ? "#c9a84c" : "#c0392b" }}>{pct}%</span></div></div>
                <div className={clsx("h-2 rounded-full", isDark ? "bg-[#6b5349]" : "bg-[#e2e8f0]")} role="progressbar" aria-valuenow={pct} aria-valuemin={0} aria-valuemax={100} aria-label={`${area} fulfillment`}><div className="h-full rounded-full transition-all" style={{ width: `${pct}%`, background: pct === 100 ? "#4a5e3a" : pct >= 60 ? "#c9a84c" : "#c0392b" }} /></div>
              </div>
            );
          })}
        ...
        {/* Record lineage */}
        <div className="card"><div className="card-header"><div className="flex items-center gap-2"><GitBranch className="w-4 h-4 text-[#4a8fa8]" aria-hidden="true" /><span className="card-title">Record lineage</span></div><span className="ml-auto text-[11px]" style={{ color: "var(--text-muted)" }}>How documents connect across modules</span></div><div className="card-body">
            ...
            <div className="space-y-0">
              {findings.filter((f) => f.evidenceLink?.trim()).map((f) => (
                <div key={f.id} className="flex items-start gap-3 py-3 border-b last:border-0" style={{ borderColor: isDark ? "#0f2039" : "#f1f5f9" }}>
                  <div className="flex flex-col items-center gap-1 pt-1"><div className="w-2 h-2 rounded-full bg-[#a57865]" /><div className="w-0.5 flex-1 min-h-[20px]" style={{ background: isDark ? "#6b5349" : "#e2e8f0" }} /></div>
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-2 flex-wrap"><FileText className="w-3.5 h-3.5 text-[#4a5e3a]" aria-hidden="true" /><span className="font-mono text-[11px] text-[#4a5e3a]">{f.evidenceLink}</span>
                      {f.capaId && <><ArrowRight className="w-3 h-3" style={{ color: "#8e7065" }} aria-hidden="true" /><Badge variant="amber">{f.capaId}</Badge><span className="text-[10px]" style={{ color: "var(--text-muted)" }}>CAPA Tracker</span></>}
                    </div>
                  </div>
                </div>
              ))}
      ...
      <Modal open={addDocOpen} onClose={() => setAddDocOpen(false)} title="Add document">
        <form onSubmit={docForm.handleSubmit(onDocSave)} noValidate className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2"><label htmlFor="doc-title" className={lbl} style={{ color: "var(--text-muted)" }}>Document title *</label><input id="doc-title" className="input text-[12px]" placeholder="e.g. SOP-QC-042 OOS Investigation Procedure" {...docForm.register("title")} />{docForm.formState.errors.title && <p role="alert" className="text-[11px] text-[#c0392b] mt-1">{docForm.formState.errors.title.message}</p>}</div>
      ...
      <Modal open={buildPackOpen} onClose={() => setBuildPackOpen(false)} title="Build evidence pack">
        <form onSubmit={packForm.handleSubmit(onPackSave)} noValidate className="space-y-4">
          ...
          <div><label htmlFor="pack-name" className={lbl} style={{ color: "var(--text-muted)" }}>Pack name *</label><input id="pack-name" className="input text-[12px]" placeholder="e.g. FDA 483 Response — Mumbai Site" {...packForm.register("name")} />{packForm.formState.errors.name && <p role="alert" className="text-[11px] text-[#c0392b] mt-1">{packForm.formState.errors.name.message}</p>}</div>
          <div><label htmlFor="pack-purpose" className={lbl} style={{ color: "var(--text-muted)" }}>Purpose *</label><textarea id="pack-purpose" rows={3} className="input text-[12px] resize-none" placeholder="Describe the purpose of this evidence pack..." {...packForm.register("purpose")} />{packForm.formState.errors.purpose && <p role="alert" className="text-[11px] text-[#c0392b] mt-1">{packForm.formState.errors.purpose.message}</p>}</div>
      ...
      {selectedDocs.size > 0 && (
        <div className={clsx("fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-5 py-3 rounded-2xl shadow-lg", isDark ? "bg-[#503e37] border border-[#6b5349]" : "bg-white border border-[#e2e8f0]")} style={{ boxShadow: "0 8px 24px rgba(0,0,0,0.15)" }} role="status" aria-live="polite">
          <Package className="w-4 h-4 text-[#a57865]" aria-hidden="true" />
