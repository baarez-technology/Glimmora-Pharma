...
-- Backfill createdById from the legacy createdBy display-name cache.
-- Only resolves rows where the name maps to EXACTLY ONE User in the same
-- tenant; rows with zero matches (name not a current User) or ambiguous
-- matches (duplicate display names) are left NULL on purpose and fall back
-- to name comparison in the SoD guards. Portable across SQLite and Postgres.
UPDATE "CAPA"
SET "createdById" = (
    SELECT u."id" FROM "User" u
    WHERE u."tenantId" = "CAPA"."tenantId" AND u."name" = "CAPA"."createdBy"
)
