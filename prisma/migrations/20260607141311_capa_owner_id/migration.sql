...
-- Backfill ownerId from the legacy `owner` string. The owner dropdown stores a
-- userId, so resolve an exact User.id match first; otherwise fall back to a
-- same-tenant UNIQUE display-name match (legacy/seed rows). Rows whose owner is
-- empty, not a current User, or an ambiguous name stay NULL on purpose.
-- Portable across SQLite and Postgres.
-- Pass 1 — owner already holds a valid User.id in the same tenant.
UPDATE "CAPA"
SET "ownerId" = "owner"
WHERE "ownerId" IS NULL
  ...
  AND EXISTS (
    ...
    WHERE u."id" = "CAPA"."owner" AND u."tenantId" = "CAPA"."tenantId"
  );
-- Pass 2 — owner is a display name resolving to exactly one User in the tenant.
UPDATE "CAPA"
SET "ownerId" = (
    SELECT u."id" FROM "User" u
    WHERE u."name" = "CAPA"."owner" AND u."tenantId" = "CAPA"."tenantId"
)
WHERE "ownerId" IS NULL
  AND "owner" IS NOT NULL
  AND "owner" <> ''
